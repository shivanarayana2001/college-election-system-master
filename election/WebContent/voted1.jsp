...
int a=Integer.parseInt(session.getAttribute("sportsvote").toString());
if(session.getAttribute("voteforsports").toString().equals("yes"))
{
	if(a==0)
	{
		String vid=request.getParameter("voteid");
		/*=====================*/
		ResultSet check=s.executeQuery("select contestant_id from contestant where category=\'sports\'");
		int chec=0;
		while(check.next())
		{
		...
		ps.close();
		a=99;
		PreparedStatement pss=con.prepareStatement("update user "+"set sportsvote =\'"+a+"\'where userId = \'"+iid+"\'");
		pss.execute();
