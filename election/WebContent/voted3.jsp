...
int a=Integer.parseInt(session.getAttribute("technicalvote").toString());
if(session.getAttribute("votefortechnical").toString().equals("yes"))
{
	if(a==0)
	{
		String vid=request.getParameter("voteid");
		/*=====================*/
		ResultSet check=s.executeQuery("select contestant_id from contestant where category=\'technical\'");
		int chec=0;
		while(check.next())
		{
		...
		ps.close();
		a=99;
		PreparedStatement pss=con.prepareStatement("update user "+"set technicalvote =\'"+a+"\'where userId = \'"+iid+"\'");
		pss.execute();
