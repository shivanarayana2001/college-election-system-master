...
if(session.getAttribute("votefordepartment").toString().equals("yes"))
{
	if(a==0)
	{
		...
		/*=====================*/
		ResultSet check=s.executeQuery("select contestant_id from contestant where category=\'department\'");
		int chec=0;
		while(check.next())
		{
		...
		ps.close();
		a=99;
		PreparedStatement pss=con.prepareStatement("update user "+"set departmentvote =\'"+a+"\'where userId = \'"+iid+"\'");
		pss.execute();
